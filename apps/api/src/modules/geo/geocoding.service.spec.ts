...
describe("GeocodingService", () => {
  ...
  describe("reverseGeocode", () => {
    it("should return address for valid coordinates", async () => {
      ...
      const result = await service.reverseGeocode(-23.5505, -46.6333);
    ...
    it("should throw HttpException when no address found", async () => {
      const mockResponse = {
        ok: true,
    ...
    it("should throw HttpException on HTTP error", async () => {
      ...
      await expect(service.reverseGeocode(-23.5505, -46.6333)).rejects.toThrow(
        HttpException,
      );
    });
